...
export const FreshNew = ()=>{
  ...
  const getRecipeAll = async()=>{
    try {
      const res = await fetch(`/api/v1/recipe/getall`,{
        method:"GET",
        headers:{
  ...
  return(     
    ...
    <div class='meals-div'>
      <h2 class='meals-name'>Fresh & New</h2>
      <div class='fresh-new-line'></div>
    </div>    
      <div id='breakfast-container-one' >        
        {recipes.length > 0 && recipes.slice(-3).map((recipe,i)=>{  
          let recipeUrl = '/popup?id=' + recipe._id;         
          return <>
          <div id='meal-container' key={i} onClick={togglePopup}>                         
          <Link to={recipeUrl}>                                                               
